...
def live_stream_query_manager(get_dict, req_user, return_type="html", empty_search_msg=None):
    ...
    if hist_type == "eyehistory" and hasattr(history, 'object_list'):
        ...
        if search_params.get('username') == req_user.username:
            self_profile = True
        history.object_list = group_history(
            history.object_list, req_user, self_profile=self_profile)
    elif hasattr(history, 'object_list'):
        set_tags(history.object_list, req_user)

    return hist, history_renderer(req_user,
                                  history,
                                  hist_type,
                                  return_type,
                                  empty_search_msg,
...
def history_search(req_user, timestamp=None,
                   query=None, sort="top",
                   filter="following", type=None,
                   direction="hl", orderBy="start_time",
                   limit=None, username=None,
        ...
        try:
            ...
            if query:
                history = history.filter(
                    Q(popular_history__title__icontains=query) |
                    Q(popular_history__url__icontains=query))

            if start_time and end_time:
                history = history.filter(
                    avg_time_ago__gt=start_time, avg_time_ago__lt=end_time)
            elif start_time:
            ...
            if req_user.is_authenticated():
                ...
                if len(mutelist) > 0:
                    query = reduce(
                        operator.and_, (
                            ~Q(popular_history__url__contains=x
                               ) for x in mutelist))
                    history = history.filter(query)

                mutelist = MuteList.objects.filter(
                    user=req_user, word__isnull=False
                ).values_list('word', flat=True)
                if len(mutelist) > 0:
                    query = reduce(
                        operator.and_, (
                            ~Q(popular_history__title__contains=x
                               ) for x in mutelist))
                    history = history.filter(query)
...
def profile_stat_gen(profile_user, filter=None, username=None, url=None):
    """
        ...
        If a url is present, the queryset is filtered
        to reduce the set to only this url
    """
    if username is None:
...
def _online_users():
    """
        Computes all of the users from the
        history items from the last 5 minutes
    """
    timestamp = timezone.now() - timedelta(minutes=5)

    history = EyeHistory.objects.filter(
        start_time__gt=timestamp).select_related()

    users = set()
    for h in history:
        if h.user not in users:
            users.add(h.user)
    return users
        ...
        while j < len(history):
            if group.domain == history[j].domain \
                ...
                if not self_profile and req_user.is_authenticated():
                    url = history[j].url
                    if EyeHistory.objects.filter(
                            user=req_user, url=url).exists():
                        if group.visited:
